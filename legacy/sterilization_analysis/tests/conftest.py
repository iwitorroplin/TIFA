...
encontrarlos en produccion.

NOTA (Fase 0 del port a TIFA): este conftest es una copia recortada del de
`sterilization_analysis`. Se le quita la fixture `conn` (y su import de
`steril.storage.schema`) porque ninguno de los tests portados la usa, y
`storage/` no se vendoriza -no es "nucleo": es exactamente lo que TIFA
sustituye por su propio esquema por modulo. El resto es identico al origen.
Ver legacy/sterilization_analysis/README.md.
"""
...
from steril.core.config import Autoclave, Settings, DEFAULT_SETTINGS
from steril.core.models import Series
...
@pytest.fixture
def autoclave() -> Autoclave:
    return Autoclave(id=1, name="Test", file_pattern="Test.xlsx")
